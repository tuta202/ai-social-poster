...
async def confirm_job(
    ...
    """
    Confirm job → generate Day 1 content only → set SCHEDULED.
    Days 2+ are generated by the scheduler at post time.
    SSE events:
      step: {step: "generating", message: ..., current: int, total: int}
      step: {step: "complete", message: "Day 1 ready for review!"}
    ...
    day1_post_ids = [
        p.id for p in job.posts if p.day_index == 1
    ]
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            total = len(day1_post_ids)

            yield sse("step", {
                "step": "generating",
                "message": f"Generating Day 1 content ({total} posts)...",
                "current": 0,
                "total": total,
            })
            ...
            with SessionLocal() as db_session:
                day1_posts = (
                    db_session.query(JobPost)
                    .filter(JobPost.id.in_(day1_post_ids))
                    .order_by(JobPost.post_order)
                    .all()
                )
                for post, result in zip(day1_posts, results):
            ...
            yield sse("step", {"step": "complete", "message": "Day 1 ready for review!"})
...
async def approve_post(
    ...
    if post.status not in (PostStatus.PENDING,):
        raise HTTPException(
            status_code=400,
